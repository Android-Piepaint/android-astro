---
title: 跨平臺桌寵軟體—— XR Animator，讓妳最喜歡的動漫角色動起來！支援動態捕捉和AR...
published: 2025-10-02
description: 網課時的摸魚神器！還有... SD X2 Elite 的參考設計原型機好好看！
...
:::
XR Animator是一個支援Linux、Windows、macOS的開源桌寵軟體，可以在桌面放3D模型，讓他/她與妳打招呼。內建採用了[MediaPipe](https://github.com/google/mediapipe)和[TensorFlow](https://github.com/tensorflow/tfjs-models)機器學習技術的動態捕捉，能夠實時偵測到人體的動作，並將其轉化爲人物模型的動作。
...
# 基本用法
程式打開後，將妳的模型（需包含有`.pmx` 檔的ZIP壓縮文件）直接拖入窗口，然後按下“START”來啓動！（沒有模型就跳過這個步驟）
...
可以使用攝像頭進行實時的動作捕捉，亦可以通過載入單張照片來使人物做出想要的姿勢。某些動作後面會有對應的表情展示，來讓妳來決定是進行全身捕捉或半身捕捉。

![Action menu](assets/action-menu.png)

首先，設定要使用的照相機，按一下工具列的“相機”設定，在對話框中選擇妳的相機。然後點選“動作”設定，選擇第三個，按一下“動態捕捉”設定身體部分。之後人物會根據妳的面部表情和動作跟着反應了。
