---
title: 準備開刷！(準備, 始作, 플래싱!)(Feat. Kryo Kodiak) -- ‘Blue Archive’ 中國 서버(server) 大運動會 主題曲 ‘出發準備!(출발준비)’를 FOSS가 패러디한 버전 (韓國語 버전, Korean Ver.)
published: 2026-04-30
description: 讓我們在一起，構建新的篇章！寫給我的 QCS6490 單板機和 SM8750 MTP 原型機。
image: 'assets/mtp8750-front-view.jpg'
tags: [FOSS, Lyric, Korean]
category: 'Music'
draft: false
lang: 'en'
---


<audio controls>
  <source src="https://github.com/Android-Piepaint/android-astro-files/raw/refs/heads/main/Ready,%20Set,%20Flash!%20(%EC%A4%80%EB%B9%84,%20%EC%8B%9C%EC%9E%91,%20%ED%94%8C%EB%9E%98%EC%8B%B1!).mp3" type="audio/mpeg">
  Your browser does not support the audio element.
</audio>

<iframe width="560" height="315" src="https://www.youtube.com/embed/DLzUDHSYUWM?si=HR8Gia2XaX4PUVBb" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

## Chorus

팬(fan) 소리 높여봐,</br>
世上(세상) 中心(중심)에서 </br>

> 讓風扇更響亮，站在世界中央

우리 함께 모여, </br>
自由(자유)를 외쳐봐! (헤이!)</br>

> 我們聚在一起，高喊自由口號！（嘿！）

나 리 라 바 라 바, 準備(준비)되면 플래싱(flash)!</br>
나 리 라 바 라 바, 오픈 소스(Open source) 빛 아래!</br>

> Na Li La Ba La Ba，準備好了就開刷！</br>
> Na Li La Ba La Ba，在開源之光照耀下！</br>

망설이지 말고 발을 내디뎌,</br>
靈感(영감)을 펼쳐봐, 같이 빌드(build)해!</br>

> 不要猶豫，邁出第一步,</br>
> 展開靈感，一起編譯吧！</br>

## Verse

제자리에, 프로브(probe)!</br>
`Preloader` 뒤엔 `DA`!</br>

> 各就位，探針準備！</br>
> `Preloader` 之後接 `DA`！</br>

리테일(retail)은 , 퓨즈(fuse) 펑!</br>
壁(벽)돌 되면 補償(보상)해!</br>

> 零售就位，熔絲熔燬!</br>
> (裝置)變磚記得賠！</br>

퀄컴(Qualcomm) 展示會(전시회),</br>
寶物(보물)이 가득!</br>
몰라도 상관없어!</br>

> 高通展示會，滿滿寶貝！</br>
> 不懂也無所謂！</br>

포팅(port) 힘들면,</br>
잠시 잊어둬,</br>
다음 機器(기기) 누구지?</br>
> 移植(port)太難就先放置一邊，</br>
> 下一臺裝置又是誰？</br>

## Bridge

La la la, la la la la la la </br>
La la la, la la la la la la la la la </br>
La la la, la la la la la la </br>
La la la, la la la la la la la </br>

## Verse

로그(log) 속에서 너를 찾아봐,</br>
너는 봤을까 나의 LED </br>

> 在 log 中尋找妳的身影，</br>
> 是否曾看清，我努力在閃燈致意</br>

어두운 畵面(화면) 속,</br>
흐릿한 너의 모습 </br>

> 在黑暗畫面中黯淡的妳，</br>

커널(kernel)이 돌아가!</br>

> 核心開始運作了！</br>

## Chorus

텐션을 높여봐,</br>
하드웨어(hardware) 모두 OK!</br>

> 讓情緒更高漲，</br>
> 所有硬體工作正常！</br>

靈感(영감)을 모아서, </br>
피워봐!</br>

> 讓靈感聚集，</br>
> 盡情綻放！</br>

프라이버시(privacy)의 夢(몽),</br>
人權(인권)이 保障(보장)된 곳</br>

> 滿懷捍衛隱私的夢想,</br>
> 數位人權得到保障 </br>

우리 함께 모여,</br>
멋진 장을 빌드해!</br>

> 讓我們在一起,</br>
> 編譯美好篇章！</br>

## Ending

나 리 라 바 라 바, 準備(준비)되면 플래싱(flash)!</br>
나 리 라 바 라 바, 오픈 소스(Open source) 빛 아래!</br>

> Na Li La Ba La Ba，準備好了就開刷！</br>
> Na Li La Ba La Ba，在開源之光照耀下！</br>

망설이지 말고 발을 내디뎌,</br>
靈感(영감)을 펼쳐봐, 같이 빌드(build)해!</br>

> 不要猶豫，邁出第一步,</br>
> 展開靈感，一起編譯吧！</br>

靈感(영감)을 펼쳐봐, 같이 빌드(build)해!</br>

> 展開靈感，一起編譯吧！</br>