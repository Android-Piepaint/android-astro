...
但是，不知道聯想在韌體裏做了什麼手腳，在更新到 NHCN60WW 版的 UEFI 韌體後，下載超過1GB的檔案時，系統會突然 freeze 掉，然後重啓。我也問過聯想的技術支援，請求提供舊版韌體下載，我自行刷回舊版韌體就可以，但是支援告訴我說我的筆電並沒有韌體更新，還好奇的問我是從哪裏獲得韌體更新的（這些專業的「技術支援」難道不知道自家筆電UEFI韌體是通過 Windows Update 提供的嗎？），最終也沒有給我提供舊版韌體下載。既然筆電廠家的「技術支援」也束手無策的話，唯一的解決方法，就只有使用者自己了。

# 使用外置 USB 無線網路卡不是一個長久的方案

對於無線網路這一問題，我曾想過買一個廉價的USB網路卡取代筆電上內建的 Quectel NCM825A PCIE 網路卡。但是檢索了一圈後發現在售的USB網路卡不是外形太大，就是網路速度不佳，而且對 Linux 和 FreeBSD 這種類 UNIX 作業系統相容性不友好，因此這種方法雖然可以解決「燃眉之急」，卻不是長久的方法。</br>
...
準備三根較長的杜邦線和一個 CH341 燒錄器，將線按照主機板上的絲印進行組裝，然後將燒錄器鏈接電腦或者其他裝置上。嵌入式系統不講究所謂「上位機」的概念，之後安裝 `minicom` 並執行 `sudo minicom -s` 來設定序列阜和鮑率爲115200：</br>
...
B -    644892 - PM: Reset Type: Hard Reset
```
結合之前日用原型機的經驗與 Lumo 的判斷（我討厭Google，不要他追蹤我！），這是 CPU 被 PMIC 強制拉低電平（PS_HOLD）導致了硬重啓的發生，與作業系統及軟體沒有關係。在高通平臺上，能夠拉`PS_HOLD`的「實體」只有牠的「遠端處理器」了，再結合下載檔案時當機的特點，那毫無疑問就是無線遠端處理器的問題了（無線遠端處理器的問題，我只遇到過一次，是在一加6T上用 postmarketOS，嘗試把內建的無線網路卡切換到監聽模式檢索周圍的AP，然後當機進入Crashdump）。只是無線網路卡的 UART 測試點太小，想要鏈接杜邦線就需要自己焊接，這是比較危險的做法。因此，網路卡的韌體到底發生了什麼，我可能無法知曉，但是這個問題依然可以解決。</br>
...
最後我通過網路上有人分享的 Slim 7x 的 UEFI 韌體，通過燒錄器刷入後，再次通過UART登入系統，下載同樣的 Debian 映像。但是並沒有多少改善，並且當機重啓後會遇到PCIE裝置初始化失敗的問題：
...
遇到的問題完全不同，那是否可以套用這一問題的解決方法呢？我決定試一下。首先，前往 [Linaro 的GitLab ](https://git.codelinaro.org/clo/ath-firmware/ath12k-firmware/-/tree/main?ref_type=heads)下載網路卡的韌體，然後安裝牠們，重開機。</br>
我知道這個決定有點像是在「碰運氣」，因爲我沒有辦法改動核心對硬體的初始化流程，更不可能對韌體做除錯，換韌體基本上就是最便宜、也最容易回復的實驗。至少在風險可控的前提下，可以先排除「韌體本身壞掉」這個可能性。其次，既然 `dmesg` 和 `debugfs` 都指向韌體崩潰，我更傾向把問題先歸類爲「韌體或其載入流程的錯」，而不是 PCIe 本身的硬體瑕疵。</br>
實際動手前，我先做了幾個小準備：把現有韌體完整備份，並記下當時的核心版本與模組載入順序；同時把能穩定重現問題的測試流程寫清楚（例如固定下載同一個大檔、記錄重開機前的 `dmesg` 與 `journalctl -k` 片段）。這樣就算更換韌體後出現新問題，也能很快回到原狀，或者把變化縮小到可比對的範圍。</br>
...
這裡的體感很矛盾：速度變慢，但穩定性顯著提升。這讓我更確信是韌體層面的「崩潰」而非單純訊號或路由器造成的問題。雖然這不是完美解（畢竟性能仍不理想），但至少把系統從「會突然重開機」變成「可以正常工作」。對我來說，這一步是把問題從「災難級」降到「可忍受」，讓我可以繼續觀察並等待上游修正。</br>
...
[^1]:[有關的問題反饋](https://bugs.launchpad.net/ubuntu/+source/linux/+bug/2138754)
